  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-earth-900 flex items-center space-x-3">
            <Wallet className="w-8 h-8 text-nature-600" />
            <span>Gelir Gider Analizi</span>
          </h1>
          <p className="text-earth-500 font-medium mt-1">İşletmenizin finansal durumunu detaylı inceleyin</p>
        </div>
        <button 
      ...
      {/* Filters & Settings */}
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-earth-200 grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
        <div className="space-y-1">
          <label className="text-xs font-bold text-earth-500 uppercase flex items-center space-x-1">
          ...
          <select 
            ...
            className="w-full p-2 border border-earth-300 rounded-lg focus:ring-2 focus:ring-nature-500 font-medium"
          >
        ...
        <div className="space-y-1">
          ...
          <div className="flex space-x-2">
            ...
            <button 
              ...
              className="px-3 bg-earth-100 hover:bg-earth-200 text-earth-700 font-bold rounded-lg transition whitespace-nowrap"
            >
              Uygula
            </button>
          </div>
      ...
      {groupFilter !== 'all' && (
        <div className="bg-blue-50 border border-blue-200 p-3 rounded-xl flex items-start space-x-2 text-blue-800 text-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-earth-200 flex flex-col justify-center">
          <div className="flex items-center space-x-3 mb-2">
            <div className="p-2 bg-green-100 text-green-600 rounded-lg">
              <TrendingUp className="w-6 h-6" />
            </div>
            <h3 className="text-lg font-bold text-earth-700">Toplam Gelir</h3>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-earth-200 flex flex-col justify-center">
          <div className="flex items-center space-x-3 mb-2">
            ...
            <h3 className="text-lg font-bold text-earth-700">Toplam Gider</h3>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Gelir Kalemleri */}
        <div className="bg-white rounded-2xl shadow-sm border border-earth-200 overflow-hidden flex flex-col">
          <div className="p-4 bg-earth-50 border-b border-earth-200">
            <h3 className="font-bold text-earth-800 text-lg">Gelir Kalemleri Detayı</h3>
          </div>
          <div className="p-4 space-y-4 flex-1">
            <div className="flex justify-between items-center p-3 bg-green-50/50 rounded-xl border border-green-100">
              <div>
                <p className="font-bold text-earth-800">Süt Satışı</p>
                <p className="text-xs text-earth-500">Toplam {calculations.toplamLitre.toFixed(1)} Lt (x {sutLitreFiyati}₺)</p>
              </div>
              <p className="font-black text-green-600">{formatMoney(calculations.sutGeliri)}</p>
            </div>
            <div className="flex justify-between items-center p-3 bg-green-50/50 rounded-xl border border-green-100">
              <div>
                <p className="font-bold text-earth-800">Hayvan Satışı</p>
                <p className="text-xs text-earth-500">Formdan satıldı işaretlenenler</p>
              </div>
              <p className="font-black text-green-600">{formatMoney(calculations.hayvanSatisGeliri)}</p>
            </div>
            <div className="flex justify-between items-center p-3 bg-green-50/50 rounded-xl border border-green-100">
              <div>
                <p className="font-bold text-earth-800">Ek Gelirler</p>
                <p className="text-xs text-earth-500">Manuel eklenen gelirler</p>
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-earth-200 overflow-hidden flex flex-col">
          <div className="p-4 bg-earth-50 border-b border-earth-200">
            <h3 className="font-bold text-earth-800 text-lg">Gider Kalemleri Detayı</h3>
          </div>
          <div className="p-4 space-y-4 flex-1">
            <div className="flex justify-between items-center p-3 bg-red-50/50 rounded-xl border border-red-100">
              <div>
                <h4 className="font-bold text-earth-800">Yem Maliyeti</h4>
            ...
            <div className="flex justify-between items-center p-3 bg-red-50/50 rounded-xl border border-red-100">
              <div>
                <p className="font-bold text-earth-800">Sağlık Giderleri</p>
                <p className="text-xs text-earth-500">İlaç, aşı ve müdahale masrafları</p>
              </div>
              <p className="font-black text-red-600">-{formatMoney(calculations.saglikGideri)}</p>
            </div>
            <div className="flex justify-between items-center p-3 bg-red-50/50 rounded-xl border border-red-100">
              <div>
                <p className="font-bold text-earth-800">Üreme Giderleri</p>
                <p className="text-xs text-earth-500">Tohumlama vb. masraflar</p>
              </div>
              <p className="font-black text-red-600">-{formatMoney(calculations.uremeGideri)}</p>
            </div>
            <div className="flex justify-between items-center p-3 bg-red-50/50 rounded-xl border border-red-100">
              <div>
                <p className="font-bold text-earth-800">Ek Giderler</p>
                <p className="text-xs text-earth-500">Manuel eklenen giderler</p>
              </div>
      ...
      {calculations.ekList.length > 0 && (
        <div className="bg-white rounded-2xl shadow-sm border border-earth-200 overflow-hidden">
          <div className="p-4 bg-earth-50 border-b border-earth-200">
            <h3 className="font-bold text-earth-800">Ek Gelir ve Gider Geçmişi</h3>
          </div>
          <div className="divide-y divide-earth-100">
            {calculations.ekList.map(islem => (
              <div key={islem.id} className="p-4 flex items-center justify-between hover:bg-earth-50 transition">
                <div>
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm font-medium text-earth-500">{new Date(islem.tarih).toLocaleDateString('tr-TR')}</span>
                  </div>
                  {islem.aciklama && <p className="text-earth-700 mt-1">{islem.aciklama}</p>}
                </div>
