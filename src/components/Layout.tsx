...
const Layout: React.FC = () => {
  ...
  const NavContent = ({ isMobile }: { isMobile?: boolean }) => (
    <nav className={`flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar ${isMobile ? 'p-4 flex flex-col justify-between gap-2.5' : 'px-3 py-1 flex flex-col justify-center gap-1'}`}>
      {NAV_ITEMS.map((item) => (
        <NavLink
          key={item.path}
          to={item.path}
          ...
          className={({ isActive }) => `
            ...
            ${isMobile ? 'px-4 py-3 sm:py-3.5 text-base shadow-xs' : 'px-4 py-2 text-[14.5px]'}
            ${isActive 
