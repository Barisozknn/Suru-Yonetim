    ...
    <div className="fixed inset-0 bg-earth-900/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-xl">
        <div className="flex justify-between items-center p-6 border-b border-earth-100">
          <div>
            <h2 className="text-xl font-bold text-earth-800">{grup.ad} - Hayvan Ekle/Çıkar</h2>
        ...
        <div className="p-4 border-b border-earth-100 bg-earth-50">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-white border border-earth-200 rounded-xl focus:ring-2 focus:ring-nature-500 outline-none"
            />
          </div>
        </div>
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      <div className={`w-6 h-6 rounded border flex items-center justify-center
                        ${isCurrentlyInGroup ? 'bg-nature-500 border-nature-500' : 'border-earth-300 bg-white'}
                      `}>
                        {isCurrentlyInGroup && <CheckCircle className="w-4 h-4 text-white" />}
                      </div>
                      <div>
                        <div className="font-bold text-earth-800">{hayvan.kupeNo}</div>
                        <div className="text-xs text-earth-500">{hayvan.tur} • {hayvan.cinsiyet}</div>
                        {isOtherGroup && (
        ...
        <div className="p-4 border-t border-earth-100 bg-earth-50 rounded-b-2xl flex justify-end">
