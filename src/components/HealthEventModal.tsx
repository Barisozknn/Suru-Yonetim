...
const TUR_COLORS: Record<SaglikOlayiTur, string> = {
  'Muayene': 'bg-nature-100 text-nature-700 border-nature-300',
  ...
  'Diğer': 'bg-earth-100 text-earth-700 border-earth-300',
};
  ...
  return (
    <div className="fixed inset-0 z-50 bg-earth-900/60 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-4 border-b border-earth-200 flex justify-between items-center bg-nature-50 rounded-t-2xl shrink-0">
          <div>
            <h2 className="text-xl font-black text-earth-900">Sağlık Olayı</h2>
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
          ...
          <div>
            ...
            {tur === 'Operasyon' && hayvan?.cinsiyet === 'Erkek' && (
              <div className="flex items-center space-x-2 mt-4 bg-red-50 p-3 rounded-lg border border-red-200">
                <input 
                  type="checkbox" 
                  id="kisirlastirma" 
          ...
          {(tur === 'Aşı' || tur === 'İlaç') && (
            <div>
              <label className="text-sm font-bold text-earth-700 mb-1 block">
          ...
          {/* Açıklama */}
          <div>
            <label className="text-sm font-bold text-earth-700 mb-1 block">Açıklama *</label>
            <textarea value={aciklama} onChange={e => setAciklama(e.target.value)} rows={3}
              placeholder="Muayene bulguları, doz miktarı, hekim adı..."
              className="w-full p-2 border border-earth-300 rounded-lg focus:ring-2 focus:ring-nature-500 resize-none" />
          </div>
